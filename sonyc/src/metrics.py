...
def confusion_matrix_fine(Y_true, Y_pred, is_true_incomplete, is_pred_incomplete):
    """
    ...
    Y_true_complete = np.tile(
        np.logical_not(is_true_incomplete)[:, np.newaxis], (1, Y_pred.shape[1])
    )
    ...
    # The result is a (N,) vector.
    y_true_coarsened_without_incomplete = np.logical_and.reduce(
        np.logical_not(Y_true), axis=1
    )
...
def confusion_matrix_coarse(y_true, y_pred):
    ...
    # account for the case where are all missed
    if cm.shape == (1, 1):
        cm = np.array([[cm.item(), 0], [0, 0]])
    FP = cm[0, 1]
    FN = cm[1, 0]
    TP = cm[1, 1]
    return TP, FP, FN
...
def evaluate(prediction_path, annotation_path, yaml_path, mode):
    ...
    # Parse ground truth.
    gt_df = parse_ground_truth(annotation_path, yaml_path).sort_values("audio_filename")

    # Parse predictions.
    if mode == "fine":
        pred_df = parse_fine_prediction(prediction_path, yaml_path)
    elif mode == "coarse":
        pred_df = parse_coarse_prediction(prediction_path, yaml_path).sort_values(
            "audio_filename"
        )
    ...
    for coarse_id in yaml_dict["coarse"]:
        ...
        if mode == "coarse":
            ...
            columns = [
                ...
                if (str(column).startswith(str(coarse_id)))
                and ("-" in str(column))
                and (not str(column).endswith("X"))
            ]
        ...
        if mode == "fine":
            ...
            for i, threshold in enumerate(thresholds):
                ...
                is_pred_incomplete = pred_df[incomplete_tag].values >= threshold
...
def micro_averaged_auprc(df_dict, return_df=False):
    ...
    # Build DataFrame from columns.
    eval_df = pd.DataFrame({"threshold": thresholds, "TP": TPs, "FP": FPs, "FN": FNs})
...
def macro_averaged_auprc(df_dict, return_classwise=False):
    ...
    if return_classwise:
        class_auprc = {
            coarse_id: auprc for coarse_id, auprc in zip(coarse_id_list, auprcs)
        }
...
def parse_coarse_prediction(pred_csv_path, yaml_path):
    ...
    rev_coarse_dict = {
        "_".join([str(k), yaml_dict["coarse"][k]]): k for k in yaml_dict["coarse"]
    }
...
def parse_fine_prediction(pred_csv_path, yaml_path):
    ...
    for coarse_id in yaml_dict["coarse"]:
        ...
        if incomplete_tag not in fine_dict.keys():
            pred_fine_dict[incomplete_tag] = np.zeros((n_samples,)).astype("int")
...
def parse_ground_truth(annotation_path, yaml_path):
    ...
    gt_df = ann_df[(ann_df["split"] == "split2")]
