...
def load_embeddings(file_list, emb_dir):
    ...
    for idx, filename in enumerate(file_list):
        filename = filename.split("/")[-1].rstrip(".wav")
        emb_path = os.path.join(emb_dir, filename + ".npz")
        embedding = np.load(emb_path)["embedding"]
        diff = embedding.shape[0] - 11
        if diff > 0:  # trim
            embedding = embedding[
                math.floor(diff / 2) : embedding.shape[0] - math.ceil(diff / 2)
            ]
        elif diff < 0:  # padding
            diff = abs(diff)
            embedding = np.pad(
                embedding,
                ((math.ceil(diff / 2), math.floor(diff / 2)), (0, 0)),
            )
...
def get_subset_split(annotation_data):
    ...
    data = (
        annotation_data[["split", "audio_filename", "horn"]]
...
def get_file_targets(annotation_data, labels):
    ...
    labels = ["person", "bicycle", "car", "motorcycle", "siren", "bus", "truck"]
    ...
    for _, row in annotation_data.iterrows():
        ...
        #     continue
        for label in labels:
            count_dict[fname][label] += row[label]

    targets = np.array(
        [
            [1.0 if count_dict[fname][label] > 0 else 0.0 for label in labels]
...
def construct_mlp(
    ...
    for idx in range(num_hidden_layers):
        y = TimeDistributed(
            ...
            input_shape=(num_frames, repr_size),
        )(y)
...
def prepare_data(
    ...
    X_train = X_train_emb  # np.concatenate((X_train_cts, X_train_time), axis=-1)
    X_valid = X_valid_emb  # np.concatenate((X_valid_cts, X_valid_time), axis=-1)
    X_test = X_test_emb  # np.concatenate((X_test_cts, X_test_time), axis=-1)
...
def train_model(
    ...
    cb.append(keras.callbacks.CSVLogger(history_csv_file, append=True, separator=","))
...
def train(
    ...
    annotation_data = pd.read_csv(annotation_path, index_col=0).sort_values(
        "audio_filename"
    )
    ...
    fine_target_labels = [
        x for x in full_fine_target_labels if x.split("_")[0].split("-")[1] != "X"
    ]
    coarse_target_labels = [v for k, v in taxonomy["coarse"].items()]
    print(taxonomy["fine"])
    print(taxonomy["coarse"])
    ...
    X_train, y_train, X_valid, y_valid, X_test, y_test, scaler = prepare_data(
        train_file_idxs,
        valid_file_idxs,
        test_file_idxs,
        embeddings,
        ...
        def masked_loss(y_true, y_pred):
            ...
            for coarse_idx in range(len(full_coarse_to_fine_terminal_idxs)):
                ...
                if true_incomplete_subidx is None:
                    ...
                    assert (
                        true_end_idx - true_start_idx == pred_end_idx - pred_start_idx
                    )
...
def generate_output_file(
    filename, y_pred, file_idxs, results_dir, file_list, label_mode, taxonomy
):
    """
    ...
    full_fine_target_labels = [
        fine_label for fine_list in coarse_fine_labels for fine_label in fine_list
    ]
    ...
    with open(output_path, "w") as f:
        ...
        for (
            ...
            if label_mode == "fine":
                ...
                coarse_idx = 0
                fine_idx = 0
                for coarse_label, fine_label_list in zip(
                    coarse_target_labels, coarse_fine_labels
                ):
                    for fine_label in fine_label_list:
                        ...
                        # fine level labels
                        coarse_values[coarse_idx] = max(
                            coarse_values[coarse_idx], y[fine_idx]
                        )
                        fine_idx += 1
                    coarse_idx += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--label_mode", type=str, choices=["fine", "coarse"], default="coarse"
    )
