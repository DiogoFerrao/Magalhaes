from yolo.utils.google_utils import *  # noqa: F403
from yolo.utils.layers import *  # noqa: F403
from yolo.utils.parse_config import *  # noqa: F403
...
def load_darknet_weights(self, weights, cutoff=-1, schreder=False):
    ...
    for i, (mdef, module) in enumerate(
        ...
        if mdef["type"] == "convolutional":
            ...
            if mdef["batch_normalize"]:
                ...
                # Load conv. bias
                if schreder and (i == 143 or i == 158 or i == 173):
                    # loading 255 biases (80 classes + 5) x 3 anchors
                    # original: (255,)
                    # we want 39 biases (8 classes + 5) * 3 anchors -> (39,)
                    # hardcoded list with the indexes we want, even if we want to ignore the weights
                    load_indexes = (
                        torch.tensor(
                            [
                                0,
                    ...
                    # list with indexes which weights we want to ignore, this is indexed in the final array
                    ignore_indexes = torch.tensor(
                        [4, 6, 4 + 13, 6 + 13, 4 + 26, 6 + 26]
                    )
                    # loaded_weights: shape (39,)
                    loaded_weights = (
                        torch.from_numpy(weights[load_indexes])
                        .view_as(conv.bias)
                        .to(conv.bias.data.device)
                    )
                    random_weights = conv.bias.data
                    loaded_weights[ignore_indexes] = random_weights[ignore_indexes]

                    # headstart emergency_vehicles bias is combination of trucks with cars (50/50)
                    loaded_weights[torch.tensor([4])] = (
                        loaded_weights[torch.tensor([2])]
                        + loaded_weights[torch.tensor([7])]
                    ) / 2
                    loaded_weights[torch.tensor([4 + 13])] = (
                        loaded_weights[torch.tensor([2 + 13])]
                        + loaded_weights[torch.tensor([7 + 13])]
                    ) / 2
                    loaded_weights[torch.tensor([4 + 26])] = (
                        loaded_weights[torch.tensor([2 + 26])]
                        + loaded_weights[torch.tensor([7 + 26])]
                    ) / 2

                    conv.bias.data.copy_(loaded_weights)
                    ptr += 255
                else:
                    nb = conv.bias.numel()
            ...
            # Load conv. weights
            # We changed the last layers of each level to account for the 8 classes we use instead of 80
            # So we need to load some specific weights, this includes the weights for 8 classes and 5 output for each anchor.
            # Additionally, we want to ignore some of those weights and use the random init instead.
            if schreder and (i == 143 or i == 158 or i == 173):
                nw_per_channel = conv.weight.shape[1]
                # shape (39, 256 | 512 | 1024, 1, 1)
                nw = 255 * nw_per_channel
                load_indexes = (
                    torch.tensor(
                        [
                            0,
                ...
                # list with indexes which weights we want to ignore, this is indexed in the final array
                ignore_indexes = torch.tensor([4, 6, 4 + 13, 6 + 13, 4 + 26, 6 + 26])

                load_ptrs = torch.tensor([], dtype=torch.int32)

                for index in list(load_indexes):
                    load_ptrs = torch.cat(
                        (
                            load_ptrs,
                            torch.arange(
                                index, index + nw_per_channel, dtype=torch.int32
                            ),
                        )
                    )

                loaded_weights = (
                    torch.from_numpy(weights[load_ptrs])
                    .view_as(conv.weight)
                    .to(conv.weight.data.device)
                )
                random_weights = conv.weight.data
                loaded_weights[ignore_indexes] = random_weights[ignore_indexes]

                # headstart emergency_vehicles weights is combination of trucks with cars (50/50)
                loaded_weights[torch.tensor([4])] = (
                    loaded_weights[torch.tensor([2])]
                    + loaded_weights[torch.tensor([7])]
                ) / 2
                ...
                conv.weight.data.copy_(loaded_weights)
                ptr += nw
            else:
