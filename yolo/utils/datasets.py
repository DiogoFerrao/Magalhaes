...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, auto_size=32, disable_iter_print=False):
        self.disable_iter_print = disable_iter_print
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            self.frame += 1
            if not self.disable_iter_print:
                print(
                    "video %g/%g (%g/%g) %s: "
    ...
    def __init__(
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                _p = Path(p)  # os-agnostic

                if p.split(".")[-1].lower() in img_formats:
                    # list of images
                    f.append(p)
                elif _p.is_dir():  # dir
                    f += glob.glob(str(_p / "**" / "*.*"), recursive=True)
                    ...
                    raise Exception("%s does not exist" % _p)
            ...
            if path_filter is not None:
                self.img_files = list(
                    filter(lambda x: path_filter in x, self.img_files)
                )
            ...
            try:
                ...
                if os.path.isfile(label):
                    with open(label, "r") as f:
                        labels_list = [x.split() for x in f.read().splitlines()]
                        # # Adapt label to schreder format
                        # for i in range(len(labels_list)):
                        #     if labels_list[i][0] => 8:
                        l = np.array(labels_list, dtype=np.float32)  # labels
                if len(l) == 0:
