...
try:
    ...
    logger.info(
        (
            ...
            "via 'pip install wandb' (recommended)"
        )
    )
...
def load_datasets(path, datasets_id):
    ...
    return [list(filter(None, paths_to_datasets))[index] for index in datasets_id]
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if not opt.datasets:  # just 1 dataset
        ...
        train_path = load_datasets("./data/datasets.names", opt.datasets)
    ...
    # Model
    pretrained_checkpoint = weights.endswith(".pt")
    pretrained = weights.endswith(".weights")
    ckpt = None
    if pretrained_checkpoint:
        ...
        model = Darknet(opt.cfg).to(device)  # create
        load_darknet_weights(model, weights, schreder="schreder" in opt.cfg)
    else:
        model = Darknet(opt.cfg).to(device)  # create
    ...
    if wandb and wandb.run is None:
        ...
        wandb_run = wandb.init(
            ...
            job_type="train",
            name=save_dir.stem,
            id=ckpt.get("wandb_id")
            if "ckpt" in locals() and ckpt is not None
            else None,
        )
        ...
        if epochs < start_epoch:
            logger.info(
                (
                    ...
                    "Fine-tuning for {epochs} additional epochs."
                )
            )
    ...
    # TODO: remove this when we update coco and other datasets to follow our ids
    # for i in range(len(dataset.labels)):
    #     for j in range(dataset.labels[i].shape[0]):
    #         if dataset.labels[i][j][0] >= nc:
    #             dataset.labels[i][j][0] = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights", type=str, default="yolov4-csp.pt", help="initial weights path"
    )
    ...
    parser.add_argument(
        "--datasets", nargs="+", type=int, help="list of dataset.txt to run"
    )
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            logger.info(
                (
                    f"Start Tensorboard with 'tensorboard --logdir {opt.project}',"
    ...
    # Evolve hyperparameters (optional)
        ...
        print(
            ...
            f"Command to train a new model with these hyperparameters: $ python train.py --hyp {yaml_file}"  # noqa: E501
        )
