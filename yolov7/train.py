...
from yolov7.utils.plots import plot_images, plot_results, plot_evolution
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        # Optimizer
        if ckpt["optimizer"] is not None and not opt.distilled:
            optimizer.load_state_dict(ckpt["optimizer"])
            best_fitness = ckpt["best_fitness"]

        # EMA
        if ema and ckpt.get("ema") and not opt.distilled:
            ema.ema.load_state_dict(ckpt["ema"].float().state_dict())
            ema.updates = ckpt["updates"]

        # Results
        if ckpt.get("training_results") is not None and not opt.distilled:
            results_file.write_text(ckpt["training_results"])  # write results.txt

        # Epochs
        if not opt.distilled:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--project",
        default="/media/cache/magalhaes/vision/checkpoints",
    ...
    parser.add_argument(
        "--distilled",
        action="store_true",
        help="the pretrained model was distilled",
    )
