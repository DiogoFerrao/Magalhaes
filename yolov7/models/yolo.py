...
from yolov7.models.common import *  # noqa: F403
from yolov7.models.experimental import *  # noqa: F403
...
class Model(nn.Module):
    def __init__(
        ...
        nc=None,
        anchors=None,
        distillation_type=DistillationType.NONE,
        ...
        # Get indexes of layers to use for distillation
        self.distillation_type = distillation_type
        self.distillation_indexes = []
        if distillation_type in [
            DistillationType.BACKBONE,
            DistillationType.BACKBONE_HEAD,
            DistillationType.BACKBONE_NECK_HEAD,
        ]:
            self.distillation_indexes.append(len(self.yaml["backbone"]) - 1)
        ...
        if distillation_type in [
            ...
            neck_layers = []
            for m in self.yaml["head"]:
                if m[2] == "Concat" and len(m[0]) > 1 and m[0][-1] > 1:
                    # After the neck the head concat the previous layer and the respective neck layer
                    neck_layers.append(m[0][-1])
            self.distillation_indexes.extend(neck_layers)

        self.distillation_indexes.extend(
            list(map(lambda x: x[0], self.yaml["head"][-4:-1]))
        )
    ...
    def forward(self, x, distillation=False, augment=False, profile=False):
        if augment:
            ...
            return self.forward_once(
                x, distillation, profile
            )  # single-scale inference, train

    def forward_once(self, x, distillation=False, profile=False):
        y, dt = [], []  # outputs
        ...
        for m in self.model:
            ...
            x = m(x)  # run
            if distillation and m.i in self.distillation_indexes:
                distillation_outputs.append(x)
