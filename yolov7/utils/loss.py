...
def normalize_obj(tensor):
    tensor = (tensor - tensor.min()).sum(1)
...
class ComputeKDLoss:
    ...
    def __init__(self, student_model):
        ...
        h = student_model.hyp  # hyperparameters
        ...
        student_det = (
            student_model.module.model[-1]
            if is_parallel(student_model)
            else student_model.model[-1]
        )  # Detect() module
        teacher_det = (
            student_model.module.model[-1]
            if is_parallel(student_model)
        ...
        self.balance = {3: [4.0, 1.0, 0.4]}.get(
            student_det.nl, [4.0, 1.0, 0.25, 0.06, 0.02]
        )  # P3-P7
        ...
        self.student_anchors = student_det.anchors
        self.teacher_anchors = teacher_det.anchors

    def __call__(
        self, p, targets, teacher_attention_map
        ...
        # Losses
        for i in range(len(p)):  # layer index, layer predictions
            pi = p[i]
            ti = targets[i]
            ps = pi.flatten(end_dim=-2)
            ts = ti.flatten(end_dim=-2)
            # repeat attention map (b, h, w) for the 3 anchors and flatten it
            teacher_attention_map_flatten = (
                teacher_attention_map[i].unsqueeze(1).repeat(1, 3, 1, 1).flatten()
            )
            # repeat anchor (3, 2) for each grid cell (h, w) and then batch (b)
            student_anchors = torch.repeat_interleave(
                self.student_anchors[i], pi.shape[2] * pi.shape[3], dim=0
            ).repeat(pi.shape[0], 1)
            teacher_anchors = torch.repeat_interleave(
                self.teacher_anchors[i], pi.shape[2] * pi.shape[3], dim=0
            ).repeat(pi.shape[0], 1)
            # obs_map = normalize_obj(ti[..., 4]).unsqueeze(1).repeat(1, 3, 1, 1).flatten()
            # b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx
            # Regression
            pxy = ps[:, :2].sigmoid() * 2.0 - 0.5
            pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * student_anchors
            pbox = torch.cat((pxy, pwh), 1)  # predicted box

            txy = ts[:, :2].sigmoid() * 2.0 - 0.5
            twh = (ts[:, 2:4].sigmoid() * 2) ** 2 * teacher_anchors
            tbox = torch.cat((txy, twh), 1)  # predicted box
            iou = bbox_iou(
                pbox.T, tbox, x1y1x2y2=False, CIoU=True
            )  # iou(prediction, target)
            lbox += (
                (1.0 - iou) * teacher_attention_map_flatten  # obs_map
            ).mean()  # iou loss

            # Classification
            lcls += (
                self.BCEcls(ps[:, 5:], ts[:, 5:].sigmoid()).mean(dim=1)
                * teacher_attention_map_flatten  # obs_map
            ).mean()  # BCE
            obji = self.BCEobj(pi[..., 4], ti[..., 4].sigmoid())
            lobj += obji * self.balance[i]  # obj loss

        lbox *= self.hyp["box"]
        lobj *= self.hyp["obj"]
        lcls *= self.hyp["cls"]
        bs = targets[0].shape[0]  # batch size
...
#         batch_size = student_features[0].shape[0]
#         loss = torch.zeros(1, device=device)
#         _, tbox, indices, anchors = self.build_targets(teacher_pred, targets)  # targets
#         # Losses
#         for i, pi in enumerate(teacher_pred):  # layer index, layer predictions
#             grid_size_y = teacher_features[i].shape[-2]
#             grid_size_x = teacher_features[i].shape[-1]
#             b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx
#             teacher_pred_i = teacher_pred[i]
#             n = b.shape[0]  # number of targets
#             fgfi_mask = torch.meshgrid([torch.zeros(grid_size_y), torch.zeros(grid_size_x)], indexing="ij")[0].unsqueeze(0).repeat(batch_size, 1, 1).clone().to(device)

#             if n:
#                 teacher_pred_selected = teacher_pred_i[b, a, gj, gi]
#                 # Regression
#                 if self.grid[i].shape[2:4] != teacher_features[i].shape[-2:]:
#                     self.grid[i] = self._make_grid(grid_size_y, grid_size_x).to(device)
#                 pxy = (teacher_pred_selected[:, :2].sigmoid() * 2.0 - 0.5)
#                 # pxy_image_space = (pxy + self.grid[i][0, 0, gj, gi]) * self.stride[i]
#                 pwh = (teacher_pred_selected[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
#                 pbox = torch.cat((pxy, pwh), 1)  # predicted box
#                 iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False)  # iou(prediction, target)
#                 fgfi_threshold = iou.max(0)[0] * self.iou_threshold  # foreground-background indicator mask
#                 iou_mask = iou > fgfi_threshold
#                 filtered_gi = gi[iou_mask]
#                 filtered_gj = gj[iou_mask]
#                 filtered_batch_items = b[iou_mask]
#                 fgfi_mask[filtered_gj, filtered_gi] = 1.0

#                 # Compute losses
#                 N = fgfi_mask.sum()
#                 loss += (fgfi_mask * (F.normalize(student_features[i]) - F.normalize(teacher_features[i])) ** 2).sum() / (2 * N)
#         return loss / (3.0 * batch_size)
