...
def test(
    ...
    save_stats=False,  # save stats.csv
    plots=True,
    wandb_logger=None,
    compute_loss=None,
    half_precision=True,
                        ...
                        for j in (ious > iouv[0]).nonzero(as_tuple=False):
                            ...
                            if d.item() not in detected_set:
                                ...
                                if (
                                    ...
                                    break
                if save_stats:
                    with open(save_dir / "stats.csv", "a+") as fp:
                        # write header in first line
                        if fp.tell() == 0:
                            fp.write(
                                "path,correct,predicted,targets,thresholds_correct,thresholds_total\n"
                            )
                        thresholds_correct = correct.sum().item() if len(correct) else 0
                        thresholds_total = torch.numel(correct) if len(correct) else 0
                        num_correct = correct[:, 0].sum().item() if len(correct) else 0
                        num_predicted = len(detected)
                        num_targets = nl
                        fp.write(
                            f"{path},{num_correct},{num_predicted},{num_targets},{thresholds_correct},{thresholds_total}\n"
                        )
    ...
    if len(stats) and stats[0].any():
        ...
        p_all, r_all, ap50_all, ap_all = (
            np.concatenate((p[:other_cls_id], p[other_cls_id + 1 :])),
            np.concatenate((r[:other_cls_id], r[other_cls_id + 1 :])),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save-stats",
        action="store_true",
        help="save stats for each image in a stats.csv file",
    )
    parser.add_argument(
        "--save-json",
        action="store_true",
    ...
    parser.add_argument(
        "--project", default="/media/magalhaes/vision/test", help="save to project/name"
    )
    ...
    # /media/magalhaes/vision/pretrained/yolov7.pt
    exp_name = (
