...
class STFT(DFTBase):
    ...
    def forward(self, input):
        ...
        if self.center:
            ...
            # TensorRT has a buggy support for F.pad, so we use a workaround
            x = torch.cat(
                (
                    x[..., 1 : (self.n_fft // 2) + 1].flip(-1),
                    x,
                    x[..., -(self.n_fft // 2) - 1 : -1].flip(-1),
                ),
                dim=-1,
            )
    ...
    def __init__(
        ...
        Args:
            ...
            frames_num: None | int, number of frames of audio clips to be
                inferneced. Only usable when onnx=True.
            device: None | str, device of ONNX. Only usable when onnx=True.
        """
...
class LogmelFilterBank(nn.Module):
    def __init__(
        ...
        self.melW = librosa.filters.mel(
            sr=sr, n_fft=n_fft, n_mels=n_mels, fmin=fmin, fmax=fmax, norm=None
        ).T
        # self.melW = AF.melscale_fbanks(n_fft // 2 + 1, n_mels, fmax, n_mels, sr).T
        # (n_fft // 2 + 1, mel_bins)
...
class LibrosaMelSpectrogram(nn.Module):
    def __init__(
        ...
        self.log_mel_filter = LogmelFilterBank(
            sr=sample_rate, n_mels=n_mels, n_fft=n_fft, is_log=False
        )

    def forward(self, x):
        x = self.spectrogram(x)
        x = self.log_mel_filter(x)[0]
        # change (b, t, n_mels) -> (b, n_mels, t)
        x = x.transpose(1, 2)
        return x
